...
def bfloat16_quantize(
    model_name: str,
    target_weight_name: List[str] = None,
    is_tied: bool = False,
    bf16_model_path: str = None
) -> None:
    ...
    Args:
        ...
        target_weight_name (str):
            name of weight name to apply converting fp32 to bf16 for reducing model storage,
            default value is model.embed_tokens.weight, indicating word embedding layers weight

        is_tied (bool): flag value of sharing embedding weight option from input model
    ...
    checkpoint_name = model_name.split("/")[2].split("-")[0]  # get quantized information from file name
    onnx_model = onnx.load(f"{model_name}{checkpoint_name}_model.onnx")
    ...
    # save the converted model to local disk
    onnx.save(onnx_model, f"{bf16_model_path}{checkpoint_name}_model.onnx")
    return
